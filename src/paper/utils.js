...
const sorageUrl =
    "https://9tvgcnacn7.execute-api.eu-central-1.amazonaws.com/default/mimic";
...
export const bind = () => {
    getItems().forEach(async (item) => {
        if (item.data.type === "rest") {
            const res = await fetch(item.data.url).then((response) =>
                response.json()
            );
...
export const updateItem = function (item, values) {
    if (values.position && item.position) {
        ...
        item.tweenTo({ position: newPosition }, 300);
    }
    ...
    Object.keys(values)
        ...
        .forEach((key) => {
            if (item[key] !== undefined) {
                item.tweenTo({ [key]: values[key] }, 300);
            }
        });
...
export const addGrid = () => {
    ...
    const drawGridVerticalLines = ({ color, width, height }) => {
        let lines = [];
    ...
    const drawGridHorizontalLines = ({ color, width, height }) => {
        let lines = [];
...
export const load = async () => {
    ...
    const mimicName =
        localStorage.getItem("projectName") ?? faker.random.words();
